...
class PriceBooks_Module_Model extends Vtiger_Module_Model {
	...
	function get_pricebook_productmodel($recordModel, $relatedModuleModel) {
		$query = 'SELECT vtiger_productmodel.productmodelid, vtiger_productmodel.productname, vtiger_productmodel.productcode, vtiger_productmodel.commissionrate,
						vtiger_productmodel.qty_per_unit, vtiger_productmodel.unit_price, vtiger_crmentity.crmid, vtiger_crmentity.smownerid,
						vtiger_pricebookproductrel.listprice
				FROM vtiger_productmodel
				INNER JOIN vtiger_pricebookproductrel ON vtiger_productmodel.productmodelid = vtiger_pricebookproductrel.productid
				INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_productmodel.productmodelid
				INNER JOIN vtiger_pricebook on vtiger_pricebook.pricebookid = vtiger_pricebookproductrel.pricebookid
				INNER JOIN vtiger_productmodelcf on vtiger_productmodelcf.productmodelid = vtiger_productmodel.productmodelid
				LEFT JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.smownerid
				LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid '
				. Users_Privileges_Model::getNonAdminAccessControlQuery($relatedModuleModel->getName()) .'
				WHERE vtiger_pricebook.pricebookid = '.$recordModel->getId().' and vtiger_crmentity.deleted = 0';
