...
// echo "hiii";
	...
	protected function processWithGrouping(Mobile_API_Request $request, $response) {
        ...
        $dateFields = ['createdtime', 'modifiedtime'];
    
        foreach ($dateFields as $dateField) {
            if (isset($resultRecord[$dateField])) {
                $dateString = $resultRecord[$dateField];

                // Parse the date using the server timezone
             $datetimeUTC = new DateTime($dateString, new DateTimeZone('UTC')); // UTC time zone
                $datetimeUTC->setTimezone(new DateTimeZone('Asia/Kolkata'));
                $istTime = $datetimeUTC->format('d-m-Y h:i:s A');
               
                    $resultRecord[$dateField] = $istTime;
		...
		$modifiedRecord['CurrentRecordId'] = $resultRecord['id'];
		 if ($module == 'HelpDesk') {
            ...
            $recordId = explode("x", $resultRecord['id']);
		        ...
		        // Fetch Comments By Engineer
                $query = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and t_comment != ''";
                ...
                $modifiedRecord['CommentsByEngineer'] = !empty($comments) ? $comments : null;
                
                // Fetch Comments By Manager
                $mquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and m_comment != ''";
                $result = $adb->pquery($mquery, array($recordId[1]));
                $mcomments = array();
                while ($row = $adb->fetch_array($result)) {
                    $mcomments[] = array(
                        'status' => $row['status'],
                ...
                $modifiedRecord['CommentsByManager'] = !empty($mcomments) ? $mcomments : null;
                
                // Fetch Spare Part Comments By Manager
                $spquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and approvepart_comment != ''";
                $result = $adb->pquery($spquery, array($recordId[1]));
                $spcomments = array();
                while ($row = $adb->fetch_array($result)) {
                    $spcomments[] = array(
                        'status' => $row['status'],
                ...
                $modifiedRecord['SparePartCommentsByManager'] = !empty($spcomments) ? $spcomments : null;
                
                // Fetch Head of Service Comments
                $servicquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and approvepart_service != ''";
                $result = $adb->pquery($servicquery, array($recordId[1]));
                ...
                $modifiedRecord['HeadofServiceComments'] = !empty($servicecomments) ? $servicecomments : null;
				$onlyassignId = explode("x", $resultRecord['assigned_user_id']['value']);
                $modifiedRecord['Assign_user_onlyid'] = $onlyassignId[1];
            // if(!empty($resultRecord['equipment_id']['value'])){
            //     $equipmentId = explode("x", $resultRecord['equipment_id']['value']);
            //     $relequipmentid = $equipmentId[1];
            //     $equipmentIdRecord = Vtiger_Record_Model::getInstanceById($relequipmentid);
            //     $modifiedRecord['warranty_status'] = $equipmentIdRecord->get('warranty_status');
                
                // // Format warranty dates to d-m-Y
                // $warEndDate = $equipmentIdRecord->get('war_end_date');
                // if($warEndDate) {
                //     $warEndDate = date('d-m-Y', strtotime($warEndDate));
                // }
                // $modifiedRecord['warranty_end_date'] = $warEndDate;
                
                // $modifiedRecord['warranty_in_months'] = $equipmentIdRecord->get('war_in_months');
                
                // $warStartDate = $equipmentIdRecord->get('war_start_date');
                // if($warStartDate) {
                //     $warStartDate = date('d-m-Y', strtotime($warStartDate));
                // }
                // $modifiedRecord['warranty_start_date'] = $warStartDate;
                
                // $modifiedRecord['warranty_days_left'] = $equipmentIdRecord->get('days_left_in_war');
                // $onlyassignId = explode("x", $resultRecord['assigned_user_id']['value']);
                // $modifiedRecord['Assign_user_onlyid'] = $onlyassignId[1];
            // }
			...
			//add values from accounts into Equipment details api Bhagyesh
			if(!empty($resultRecord['account_id']['value'])){
    			$conrecordId = explode("x", $resultRecord['account_id']['value']);
    			$relrecordid = $conrecordId[1];
    			$orgRecord = Vtiger_Record_Model::getInstanceById($relrecordid);
    			$modifiedRecord['accountname'] = $orgRecord->get('accountname');
    			$modifiedRecord['bill_street'] = $orgRecord->get('bill_street');
    			$modifiedRecord['phone'] = $orgRecord->get('phone');
    			$modifiedRecord['bill_city'] = $orgRecord->get('bill_city');
    			$modifiedRecord['bill_state'] = $orgRecord->get('bill_state');
    			$modifiedRecord['bill_code'] = $orgRecord->get('bill_code');
    			$modifiedRecord['bill_country'] = $orgRecord->get('bill_country');
    			$modifiedRecord['account_district'] = $orgRecord->get('account_district');
    			$modifiedRecord['warranty_stage'] = $resultRecord['eq_run_war_st'];
	...
	protected function transformRecordWithOutGroupingNonDependent($resultRecord, $module, $isTemplateRecord = false, $IgRecord) {
				...
				if (isset($resultRecord[$fieldname])) {
					...
					if ($field['uitype'] == '69') {
						...
						foreach ($imageDetails as $imageDetail) {
							...
							$attachment['subject'] = $imageDetail['name'];
							array_push($attachments, $attachment);
						}
						$fields[$field['name']] = $attachments;
		...
		if ($module == 'SalesOrder') {
			...
			if (isset($resultRecord['LineItems'])) {
				$lineItems = $resultRecord['LineItems'];
				// Add serial number for each line item
                foreach ($lineItems as &$lineItem) {
                    if (!empty($lineItem['productid'])) {
                        $productId = explode('x', $lineItem['productid']);
                        $productId = $productId[1];
                        
                        // Query to get serial number from products table
                        $query = "SELECT serialno FROM vtiger_products WHERE productid = ?";
                        $result = $adb->pquery($query, array($productId));
                        
                        if ($result && $adb->num_rows($result) > 0) {
                            $lineItem['serialno'] = $adb->query_result($result, 0, 'serialno');
                        } else {
                            $lineItem['serialno'] = '';
                        }
                    } else {
