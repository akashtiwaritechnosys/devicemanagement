...
class Mobile_WS_ResetPassword extends Mobile_WS_Controller {
    ...
    function process(Mobile_API_Request $request) {
        ...
        if ($newPassword != $repeatnewPassword) {
            $response->setError(100, 'NewPassword And RepeatnewPassword Is Not Matching');
            return $response;
        }
        ...
        if ($status == true) {
            ...
            if ($otp == $otpFromDataBase) {
                ...
                if ($now > $sentTime) {
                    ...
                    $userType = $this->getUserTypeFromShortURL($shortURLModel);
                    
                    if ($userType == 'BIZUSER') {
                        $result = $this->updateCustomerPassword($shortURLModel, $newPassword);
                    } else {
                    ...
                    if (isset($result['message']) && $result['message'] == 'Changed password successfully') {
    ...
    private function getUserTypeFromShortURL($shortURLModel) {
        if (!empty($shortURLModel->handler_data['badgeNo'])) {
            return 'EPSILONUSER'; // Service Engineer
        } else {
            return 'BIZUSER'; // Customer
        }
    }
    
    /**
     * Update password for customer users
     */
    private function updateCustomerPassword($shortURLModel, $newPassword) {
        global $adb;
        
        try {
            $email = $shortURLModel->handler_data['email'];
            $userId = $shortURLModel->handler_data['id'];
            
            $hashedPassword = $this->hashPassword($newPassword);
            
            $updateSql = "UPDATE vtiger_users SET user_password = ?, confirm_password = ? WHERE user_name = ?";
            $result = $adb->pquery($updateSql, array($hashedPassword, $hashedPassword, $email));
            ...
            return array('success' => false, 'message' => 'Error updating password: ' . $e->getMessage());
        }
    }
