...
class HelpDesk_Detail_View extends Inventory_Detail_View {
	...
	public function showModuleDetailView(Vtiger_Request $request) {
		...
		if ($adb->num_rows($result) > 0) {
			...
			$comments[] = array('status' => '', 't_comment' => 'No status or comments found for this ticket.');
		}

		$mquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and m_comment != ''";
		$result = $adb->pquery($mquery, array($recordId));
		$mcomments = array();
		...
		if ($adb->num_rows($result) > 0) {
			...
			$mcomments[] = array('status' => '', 'm_comment' => 'No status or comments found for this ticket.');
		}

		$spquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and approvepart_comment != ''";
		$result = $adb->pquery($spquery, array($recordId));
		$spcomments = array();
		...
		if ($adb->num_rows($result) > 0) {
			...
			$spcomments[] = array('status' => '', 'approvepart_comment' => 'No status or comments found for this ticket.');
		}

		$servicquery = "SELECT * FROM vtiger_troubletickets_comments_history WHERE ticketid = ? and approvepart_service != ''";
		$result = $adb->pquery($servicquery, array($recordId));
		...
		if ($adb->num_rows($result) > 0) {
			...
			$servicecomments[] = array('status' => '', 'approvepart_service' => 'No status or comments found for this ticket.');
		}
		...
		$viewer->assign('MCOMMENTS', $mcomments);
		$viewer->assign('SPCOMMENTS', $spcomments);// Pass the comments array to the viewer
		$viewer->assign('SERVICECOMMENTS', $servicecomments);//
