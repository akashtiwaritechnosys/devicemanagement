...
abstract class Base_Chart extends Vtiger_Base_Model{
	...
	public function getDataTypes() {
		...
		foreach ($selectedDataFields as $dataField) {
			list($tableName, $columnName, $moduleField, $fieldName, $single) = array_pad(explode(':', $dataField), 5, null);
			//list($relModuleName, $fieldLabel) = explode('_', $moduleField);
			list($relModuleName, $fieldLabel) = array_pad(explode('_', (string) ($moduleField ?? '')), 2, '');
...
class VerticalbarChart extends Base_Chart {
	function generateData() {
		...
		$data = array(	'labels' => $labels,
						'values' => $values,
						'links' => $links,
						'type' => (isset($values[0]) && php7_count($values[0]) == 1) ? 'singleBar' : 'multiBar',
