...
class Users extends CRMEntity
{
	...
	{
		global $current_user, $adb, $siteID; //$adb added by raju for mass mailing
		$insertion_mode = $this->mode;

		$siteRecord = Vtiger_Record_Model::getRecordDetailsFromCentral($siteID, 'Potentials');

		$siteRecord = json_decode($siteRecord, true);

		$sitetrialstatus = $siteRecord['result']['record']['cf_875'];

		// 5 User Limit check
		// if ($sitetrialstatus == 'Active') {
		// 	$result = $adb->pquery("SELECT COUNT(*) AS user_count FROM vtiger_users WHERE deleted = 0 AND user_name != 'Admin'", []);
		// 	$userCount = $adb->query_result($result, 0, 'user_count');

		// 	if ($userCount >= 5) {
		// 		throw new Exception("User limit reached. You cannot add more than 5 users.");
		// 	}
		// }
		...
		if ($this->id) {
			...
			$email = $this->column_fields['email1'];
			global $siteID, $apiURL;

			$endpoint = '/vtigercrm/modules/Mobile/api.php';

			$Url = $apiURL . 'modules/Mobile/api.php';
			$timestamp = time();
			$sharedSecret = '2d79b2c5ba9285b85ab0e14e065c4062ad4cff5755133b9a0168f98f14fe9b51';
			$signature = hash_hmac('sha256', $timestamp . $endpoint, $sharedSecret);

			$data = json_encode([
				"_operation" => "CreateGlobalUser",
				"username" => $username,
				"email" => $email,
				"siteid" => $siteID
			]);
			...
			if ($httpCode != 200) {
				throw new AppException("Failed to create global user: " . $response);
			}
		}
	}
