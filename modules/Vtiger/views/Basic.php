...
abstract class Vtiger_Basic_View extends Vtiger_Footer_View {
	...
	function preProcess (Vtiger_Request $request, $display=true) {
		...
		// For Privacy policy accepted check
		global $current_user;
        $db = PearDatabase::getInstance();
		$result = $db->pquery("SELECT privacy_policy_accepted FROM vtiger_users WHERE id = ?", [$current_user->id]);
        if ($result && $db->num_rows($result) > 0) {
			$accepted = $db->query_result($result, 0, 'privacy_policy_accepted');
		} else {
			$accepted = 0;
		}
		$viewer->assign('PRIVACY_POLICY_ACCEPTED', $accepted);
