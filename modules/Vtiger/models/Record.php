...
class Vtiger_Record_Model extends Vtiger_Base_Model {
	...
	public static function getCurlHeaders()
	{
		...
		$timestamp = time();
		$signature = hash_hmac('sha256', $timestamp . $endpoint, $secretKey);
	...
	public static function getRecordDetailsFromCentral($record,$module)
	{
		global $apiURL,$siteID;
		$url = $apiURL.'modules/Mobile/api.php';

		$headers = self::getCurlHeaders();
		$timestamp = $headers['timestamp']; // Replace with actual timestamp
		$signature = $headers['signature']; // Replace with actual signature
		$phpsessid = '7bb8f0b681dca40aff1b'; 
