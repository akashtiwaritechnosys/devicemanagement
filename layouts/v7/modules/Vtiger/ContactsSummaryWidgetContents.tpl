					...
					<div class="">
						<div class="textOverflowEllipsis">
							<a href="{$RELATED_RECORD->getDetailViewUrl()}" id="{$MODULE}_{$RELATED_MODULE}_Related_Record_{$RELATED_RECORD->get('id')}" title="{Vtiger_Util_Helper::getRecordName($RELATED_RECORD->get('id'))}">
                                <span>{Vtiger_Util_Helper::getRecordName($RELATED_RECORD->get('id'))}</span>
							</a>
						</div>
						<div><span>{$RELATED_RECORD->getDisplayValue('email')}</p></div>
						<div class="textOverflowEllipsis" title="{$RELATED_RECORD->getDisplayValue('phone')}"><span>{$RELATED_RECORD->getDisplayValue('phone')}</span></div>
					</div>
				</li>
			</ul>
		</div>
