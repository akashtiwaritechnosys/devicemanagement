	...
	{else}
		...
		{if $FIELD_MODEL->getFieldDataType() eq 'image' || $FIELD_MODEL->getFieldDataType() eq 'file'}
			{if $MODULE neq 'Products'}
                            <div class='redColor'>
				<span>{vtranslate('LBL_NOTE_EXISTING_ATTACHMENTS_WILL_BE_REPLACED', $MODULE)}</span>
                            </div>
                        {/if}
		{/if}
		...
		{foreach key=ITER item=IMAGE_INFO from=$IMAGE_DETAILS}
			<div class="row" style="margin-top:5px;">
				{if !empty($IMAGE_INFO.url)}
					<div class="col-lg-6" name="existingImages"><img src="{$IMAGE_INFO.url}" data-image-id="{$IMAGE_INFO.id}" width="300" height="250" ></div>
					<div class="col-lg-3">
						<span class="row">
