{strip}
    ...
    {if $PRODUCT_EDITABLE}
        <td style="text-align:center;">
            <select id="{$product_warehouse}" name="{$product_warehouse}" data-selected-value='{$data.$product_warehouse}' data-fieldtype="picklist" type="picklist">
                <option value="" ></option>
                {foreach item=SECTION_VALUE from=$WAREHOUSE_VALUES}
                    <option {if trim(decode_html($data.$product_warehouse)) eq trim({$SECTION_VALUE})} selected {/if} value="{$SECTION_VALUE}" >{$SECTION_VALUE}</option>
                {/foreach}
            </select>
        </td>
    {/if}
    {if $IMAGE_EDITABLE}
        <td style="text-align:center;">
            <select id="{$sectionName}" name="{$sectionName}" data-selected-value='{$data.$sectionName}' data-fieldtype="picklist" type="picklist">
                <option value="" ></option>
                {foreach item=SECTION_VALUE from=$SECTION_VALUES}
                    <option {if trim(decode_html($data.$sectionName)) eq trim({$SECTION_VALUE})} selected {/if} value="{$SECTION_VALUE}" >{$SECTION_VALUE}</option>
                {/foreach}
            </select>
        </td>
    {/if}
    {if $PRODUCT_EDITABLE}
        <td>
                    ...
                    <div class="col-lg-10">
                        <div class="input-group" style="width:100%">
                            <input type="text" id="{$productName}" name="{$productName}" value="{$data.$productName}" class="productName form-control {if $row_no neq 0} autoComplete {/if} " placeholder="{vtranslate('LBL_TYPE_SEARCH',$MODULE)}"
                                   {if $MODULE  neq 'Potentials' and $MODULE  neq 'CreditNotes' and  $MODULE  neq 'DebitNotes'} data-rule-required=true {/if} {if !empty($data.$productName)} disabled="disabled" {/if}>
            ...
            {else}
                {if $COMMENT_EDITABLE}
                    <div><br>
                        <textarea id="{$comment}" name="{$comment}" style="width: 260px;min-height: 40px" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea>
                    </div>
                {/if}
                {if $MODULE eq 'Invoice'}
                    <div>
                        <p style="margin-top : 10px;margin-bottom: 0px">Enter Serial Number Below</p>
                        <textarea id="{$serialNumber}" name="{$serialNumber}" style="width: 260px;min-height: 40px" class="">{decode_html($data.$serialNumber)}</textarea>
                    </div>
                {/if}
            {/if}
        </td>
    {/if}
   {* <td>
        <input id="{$length}" name="{$length}" type="text" class="length smallInputBox inputElement"
               data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$length)}{$data.$length}{else}1{/if}"
               {if $QUANTITY_EDITABLE eq false} disabled=disabled {/if} />
    </td>
    <td>
        <input id="{$height}" name="{$height}" type="text" class="height smallInputBox inputElement"
               data-rule-required=true data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$height)}{$data.$height}{else}1{/if}"
               {if $QUANTITY_EDITABLE eq false} disabled=disabled {/if} />
    </td>*}
    <td>
...
{if $LIST_PRICE_EDITABLE}
    <td>
        <div>
            <input style="width: 85%" id="{$listPrice}" name="{$listPrice}" value="{if !empty($data.$listPrice)}{$data.$listPrice}{else}0{/if}" type="text"
