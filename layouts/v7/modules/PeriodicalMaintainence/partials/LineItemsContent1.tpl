	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER1}
		<td {if $tabletdhiderVal eq true} class="tabletdhider" {/if} data-td="{$LINEITEM_CUSTOM_FIELD['fieldname']}">
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '21' or $LINEITEM_CUSTOM_FIELD['uitype'] eq '2'}
				<div><textarea value="{decode_html($data.${$fieldName})}" id="{${$fieldName}}" name="{${$fieldName}}" class="">{decode_html($data.${$fieldName})}</textarea></div>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '10'}
				<div id="{$fieldName}DivCla" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>	
					<div class="referencefield-wrapper">
						<input name="popupReferenceModule" type="hidden" value="Assets"/>
                        <input id="{${$fieldName}}" name="{${$fieldName}}" type="hidden" value="{if !empty({$data[${$fieldName}]})}{{$data[${$fieldName}]}}{/if}" class="sourceField" data-displayvalue='' />
                        <div class="input-group">
							<input id="{${$fieldName}}_display" name="{${$fieldName}}_display" data-fieldname="{${$fieldName}}_display" data-fieldtype="reference" type="text" 
								class="marginLeftZero autoComplete2 inputElement" 
								value="{$data[{${$fieldName}}|cat:'_label']}"
                            {if $LINEITEM_CUSTOM_FIELD["mandatory"] eq true} data-rule-required = "true" {/if}
								placeholder="Type to Search..."/>
							<a href="#" class="clearReferenceSelection {if empty($data[${$fieldName}])}hide{/if}"> x </a>
							...
							{if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && (is_array($QUICKCREATE_RESTRICTED_MODULES) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES))}
