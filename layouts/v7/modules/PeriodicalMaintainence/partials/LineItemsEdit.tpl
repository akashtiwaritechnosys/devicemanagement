...
{strip}
	...
	<div name='editContent' class="igLineItemBlock" {if $LINEITEM_FIELDS['productid']->igReadable() eq "1"}
						...
						<div class="row">
								...
								{if $LINEITEM_FIELDS['region_id'] && $LINEITEM_FIELDS['region_id']->isEditable()}
									<span class="pull-right">
										...
										<select class="select2" id="region_id" name="region_id" style="width: 164px;">
											<option value="0"
												data-info="{Vtiger_Util_Helper::toSafeHTML(Zend_Json::encode($DEFAULT_TAX_REGION_INFO))}">
												{vtranslate('LBL_SELECT_OPTION', $MODULE)}</option>
											{foreach key=TAX_REGION_ID item=TAX_REGION_INFO from=$TAX_REGIONS}
												<option value="{$TAX_REGION_ID}"
													...
													{if $TAX_REGION_ID eq $RECORD->get('region_id')}selected{/if}>
													{$TAX_REGION_INFO['name']}</option>
											{/foreach}
										</select>
										<input type="hidden" id="prevRegionId" value="{$RECORD->get('region_id')}" />
										<a class="fa fa-wrench hidden-xs"
											href="index.php?module=Vtiger&parent=Settings&view=TaxIndex" target="_blank"
							...
							<div class="col-sm-4">
								<div class="pull-right">
									...
									<select class="select2" id="currency_id" name="currency_id" style="width: 150px;">
										{foreach item=currency_details key=count from=$CURRENCIES}
											<option value="{$currency_details.curid}" class="textShadowNone"
												data-conversion-rate="{$currency_details.conversionrate}"
												{if $SELECTED_CURRENCY.currency_id eq $currency_details.curid} selected {/if}>
									...
									<input type="hidden" value="{$SELECTED_CURRENCY.currency_id}"
							...
							<div class="col-sm-4">
								<div class="pull-right">
									...
									<select class="select2 lineItemTax" id="taxtype" name="taxtype" style="width: 150px;">
										<option value="individual" {if $IS_INDIVIDUAL_TAX_TYPE}selected{/if}>
											{vtranslate('LBL_INDIVIDUAL', $MODULE)}</option>
										<option value="group" {if $IS_GROUP_TAX_TYPE}selected{/if}>
											{vtranslate('LBL_GROUP', $MODULE)}</option>
									</select>
								</div>
							</div>
						</div>
					...
					<table class="table table-bordered" id="lineItemTab">
						<tr>
							...
							{if $PRODUCT_EDITABLE}
								<td>
									<span
										class="redColor">*</span><strong>{vtranslate({$LINEITEM_FIELDS['productid']->get('label')},$MODULE)}</strong>
								</td>
							{/if}
							{* <td>
							...
							{if $PURCHASE_COST_EDITABLE}
								<td>
									<strong
										class="pull-right">{vtranslate({$LINEITEM_FIELDS['purchase_cost']->get('label')},$MODULE)}</strong>
								</td>
							{/if}
							...
							{if $MARGIN_EDITABLE && $PURCHASE_COST_EDITABLE}
								<td>
									<strong
										class="pull-right">{vtranslate({$LINEITEM_FIELDS['margin']->get('label')},$MODULE)}</strong>
								</td>
							{/if}
			...
			<div class="fieldBlockContainer">
				<table class="table table-bordered blockContainer lineItemTable" id="lineItemResult">
					<tr>
						...
						<td>
							<div id="netTotal" class="pull-right netTotal">
								{if !empty($FINAL.hdnSubTotal)}{$FINAL.hdnSubTotal}{else}0{/if}</div>
						</td>
					</tr>
										...
										<tbody>
											<tr>
												<td><input type="radio" name="discount_final" class="finalDiscounts"
														...
														{if $DISCOUNT_TYPE_FINAL eq 'zero'}checked{/if} />&nbsp;
													{vtranslate('LBL_ZERO_DISCOUNT',$MODULE)}</td>
												<td class="lineOnTop">
											...
											{if $DISCOUNT_AMOUNT_EDITABLE}
												<tr>
													...
													<td><input type="text" data-rule-positive=true id="discount_amount_final"
															name="discount_amount_final" value="{$FINAL.discount_amount_final}"
					...
					{if $SH_PERCENT_EDITABLE}
						...
						<tr>
									...
									<table width="100%" border="0" cellpadding="5" cellspacing="0"
										...
										{foreach key=CHARGE_ID item=CHARGE_MODEL from=$INVENTORY_CHARGES}
											<tr>
												...
												<td class="lineOnTop chargeName" data-charge-id="{$CHARGE_ID}">
													{$CHARGE_MODEL->getName()}</td>
												<td class="lineOnTop">
													{if $CHARGE_MODEL->get('format') eq 'Percent'}
														<input type="text" class="span1 chargePercent" size="5" data-rule-positive=true
							...
							<td>
								<input type="hidden" class="lineItemInputBox" id="chargesTotal" name="shipping_handling_charge"
											...
											{foreach key=CHARGE_ID item=CHARGE_MODEL from=$INVENTORY_CHARGES}
												{foreach key=CHARGE_TAX_ID item=CHARGE_TAX_MODEL from=$RECORD->getChargeTaxModelsList($CHARGE_ID)}
													...
													<tr>
														...
														<td class="lineOnTop">{$CHARGE_MODEL->getName()} - {$CHARGE_TAX_MODEL->getName()}
														</td>
														<td class="lineOnTop">
															<input type="text" data-charge-id="{$CHARGE_ID}"
																...
																value="{$SH_TAX_VALUE}" data-rule-positive=true
												...
												<tr>
													...
													<td style="text-align: right;" class="lineOnTop">
														<input type="text" class="span1 deductTaxValue cursorPointer pull-right"
															name="{$DEDUCTED_TAX_INFO['taxname']}_group_amount" size="5" readonly
															value="{$DEDUCTED_TAX_INFO['amount']}" />&nbsp;
													</td>
												</tr>
											{/foreach}
										</tbody>
					...
					<tr valign="top">
						<td width="83%">
							<div style="display:flex" class="pull-right">
								...
								<div>
									<input style="width : 100%" data-rule-positive="true" max="100"
										class="lineItemInputBox form-control" type="text"
										value="{if $FINAL.tcs_per}{$FINAL.tcs_per}{else}0{/if}" name="tcs_per">
								</div>
							</div>
						</td>
						<td>
							<span class="pull-right">
								<input name="tcs_val" type="text" readonly data-rule-positive="true"
									class="lineItemInputBox form-control"
									value="{if $FINAL.tcs_per}{($FINAL.tcs_per / 100) * $FINAL.grandTotal}{else}0{/if}">
							</span>
						</td>
					</tr>
							...
							<td width="83%">
								<div style="display:flex" class="pull-right">
									...
									<div>
										<input style="width : 100%" data-rule-positive="true" max="100"
											class="lineItemInputBox form-control" type="text"
											value="{if $FINAL.tds_per}{$FINAL.tds_per}{else}0{/if}" name="tds_per">
									</div>
								</div>
							</td>
							<td>
								<span class="pull-right">
									<input name="tds_val" type="text" readonly data-rule-positive="true"
										class="lineItemInputBox form-control"
										value="{if $FINAL.tds_per}{($FINAL.tds_per / 100) * $FINAL.grandTotal}{else}0{/if}">
								</span>
							</td>
						</tr>
					{/if}
					...
					{if $MODULE eq 'SalesOrder' or $MODULE eq 'Invoice' or $MODULE eq 'PurchaseOrder' or $MODULE eq 'Bills'}
						<tr valign="top">
							<td width="83%">
								<div class="pull-right">
							...
							<td>
								{if $MODULE eq 'SalesOrder' or $MODULE eq 'Invoice'}
									<span class="pull-right"><input id="received" name="received" type="text"
											class="lineItemInputBox form-control"
											value="{if $RECORD->getDisplayValue('received') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('received')}{else}0{/if}"></span>
								{else}
									<span class="pull-right"><input id="paid" name="paid" type="text" class="lineItemInputBox"
											value="{if $RECORD->getDisplayValue('paid') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('paid')}{else}0{/if}"></span>
								{/if}
							</td>
						</tr>
						<tr valign="top">
							...
							<td>
								<span class="pull-right"><input id="balance" name="balance" type="text"
										class="lineItemInputBox form-control"
										value="{if $RECORD->getDisplayValue('balance') && !($IS_DUPLICATE)}{$RECORD->getDisplayValue('balance')}{else}0{/if}"
										readonly></span>
							</td>
						</tr>
					{/if}
