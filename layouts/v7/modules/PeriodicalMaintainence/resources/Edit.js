Inventory_Edit_Js("PeriodicalMaintainence_Edit_Js",{
    ...
    getPopUpParams: function (container) {
        ...
        if ((sourceFieldElement.attr('name') == 'assets_id' || sourceFieldElement.attr('name') == 'potential_id') && referenceModule != 'Leads') {
            var form = this.getForm();
            var parentIdElement = form.find('[name="assetname"]');
            if (parentIdElement.length > 0 && parentIdElement.val().length > 0 && parentIdElement.val() != 0) {
                var closestContainer = parentIdElement.closest('td');
                params['related_parent_id'] = parentIdElement.val();
                params['related_parent_module'] = closestContainer.find('[name="popupReferenceModule"]').val();
            } else if (sourceFieldElement.attr('name') == 'potential_id') {
                parentIdElement = form.find('[name="assets_id"]');
    ...
    searchModuleNames: function (params) {
        ...
        if (params.search_module == 'Products' || params.search_module == 'Services') {
            params.module = 'PeriodicalMaintainence';
        }
    ...
    registerReferenceSelectionEvent: function (container) {
        ...
        jQuery('input[name="assetname"]', container).on(Vtiger_Edit_Js.referenceSelectionEvent, function (e, data) {
    ...
    //     this.registerReferenceSelectionEvent1(container);
    //     this.registerEventForAssetAutofill();
    //     this.registerSerialNoChangeEvent();
    //     this.registerEventForPmEndDate();
    //     // this.registerPaymentDateValidation(); 
    //     this.clickdateIcon();
    //     $(document).ready(function() {
    
    //     $('input[name="pm_enddate"]').prop('readonly', true);
    //     $('input[name="assetname"]').prop('disabled', true);
    //     $('input[name="subject"]').prop('readonly', true);
    //     $('#PeriodicalMaintainence_editView_fieldName_assetname_select').hide();
    //     $('#for_PeriodicalMaintainence_editView_fieldName_assetname_select').hide();
        
    //     $('#PeriodicalMaintainence_editView_fieldName_no_of_installments').hide();
    //      $("td:contains('Number Of Times PM is Done')").hide();
    //     });

    // },

    registerSerialNoChangeEvent: function () {
        let thisInstance = this;
        let form = this.getForm();
        let serialNoField = form.find('[name="serial_no"]');
    
        serialNoField.on(Vtiger_Edit_Js.referenceSelectionEvent, function (e, data) {
            let recordId = data.id;
    
            if (recordId) {
                app.request.post({
                    module: 'PeriodicalMaintainence',
                    action: 'GetAssetName',
                    source_module: 'Equipment',
                    record: recordId
                }).then(function (error, responseData) {
                    if (!error && responseData.success) {
                        let assetId = responseData.data.assetname;
                        if (assetId) {
                            // Update the hidden input and display input for assetname
                            let assetNameField = form.find('[name="assetname"]');
                            assetNameField.val(assetId);
                            assetNameField.trigger('change');
                        }
                    }
                });
            }
...
//       registerPaymentDateValidation: function() {
//         var self = this;
//         var form = this.getForm();
        
//         // Delegate event handler for all payment date fields
//         form.on('change', 'input[name^="payment_date"]', function() {
//             var $this = $(this);
//             let parentRow = $this.closest('tr');
// 			let RowNumber = parseInt(parentRow.attr('data-row-num'));
//             // console.log(RowNumber);

//             var paymentDate = $this.val();
//             var pmEndDate = form.find('[name="pm_enddate"]').val();
            
//             if (paymentDate && pmEndDate) {
//                 if (!self.isDateValid(paymentDate, pmEndDate)) {
//                     $this.next('.payment-date-error').remove();
//                     var errorMessage = '<div id="validationId'+RowNumber+'" class="payment-date-error" style="color: red; font-size: 12px; margin-top: 5px;">' +
//                         'Periodic date cannot exceed the end date</div>';
//                     if(!$('#validationId'+RowNumber).length )  {
//                         $this.closest('.input-group').after(errorMessage);
//                     }
//                     $this.val('');
//                 } else {
//                     $this.closest('.input-group').next('.payment-date-error').remove();
//                     $this.next('.payment-date-error').remove();
//                 }
//             }
//         });
...
//     isDateValid: function(paymentDate, endDate) {
//         // Convert dates from dd-mm-yyyy to Date objects
//         var paymentParts = paymentDate.split('-');
//         var paymentDateObj = new Date(paymentParts[2], paymentParts[1] - 1, paymentParts[0]);
        
//         var endParts = endDate.split('-');
//         var endDateObj = new Date(endParts[2], endParts[1] - 1, endParts[0]);
        
//         // Compare payment date with end date
//         return paymentDateObj <= endDateObj;
//     },


generateSequentialDates : function (startDate, numberOfDays, step = 1) {
    const dates = [];
...
handlePaySchedule: function () {
	this.generateSequentialDates();
    let form = this.getForm();
    let years = parseInt(form.find('[name="no_of_installments"]').val());
    let pmMonth = form.find('[name="pm_month"]').val();
    if (isNaN(years)) {
        years = 0;
    }

    let otherLineItemTable = $(".lineitemTableContainerAno #lineItemTab1").clone();
    $('.lineitemTableContainerValuesHolder').html(otherLineItemTable);
    $('.lineitemTableContainerValuesHolder').addClass('hide');

    var trLength = $('body .lineitemTableContainerAno .appendRows tr').length;
    for (var i = 1; i < trLength; i++) {
        $('.lineitemTableContainerAno .appendRows tr:nth-child(2)').remove();
    }

    for (var i = 0; i <= years; ++i) {
        let date = new Date();
        let totalrow = years + 1;
        let j = i + 1;
        let html = $(".lineitemTableContainerAno .appendRows tr:first-child").clone();
        html.find('input[name^="Totalrow"]').val(totalrow);
        html.find('textarea[name^="pay_sch_sl_no"]').val(i);
        html.find('#row').removeClass('hide');
        html.find('#row').css('display', 'inline');
        html.find('#row').data('row-num', i);
        let idFields = new Array('pay_sch_sl_no', 'payment_date', 'pay_amount', 'schedule_date');
        for (let idIndex in idFields) {
			...
			if (elementId == 'schedule_date') {
                let startDate = form.find('[name="payment_start_date"]').val();
                let payment_frequency = $('select[name="payment_frequency"').val();
                if (startDate != '' && startDate != undefined && startDate != null &&
                    payment_frequency != '' && payment_frequency != undefined && payment_frequency != null
                ) {
					...
					if(payment_frequency == 'Daily') {
						...
						let fulldate = '';
						fulldate = fulldate.concat(date.getDate());
						fulldate = fulldate.concat('/');
						if (date.getMonth() < 9) {
							fulldate = fulldate.concat('0');
						}
						fulldate = fulldate.concat(date.getMonth());
						fulldate = fulldate.concat('/');
						fulldate = fulldate.concat(date.getFullYear());
						html.find('#' + expectedElementId).val(fulldate);
					} else {
						if (payment_frequency == 'Monthly') {
							...
							frequency = 12;
						} else if (payment_frequency == 'Half Yearly') {
							frequency = 6
						}
						date.setMonth(date.getMonth() + ((i * frequency) - 1 - frequency));
						let fulldate = '';
						if (date.getDate() < 10) {
							fulldate = fulldate.concat('0');
						}
						...
						fulldate = fulldate.concat(date.getMonth() + 1);
						fulldate = fulldate.concat('/');
						fulldate = fulldate.concat(date.getFullYear());
						html.find('#' + expectedElementId).val(fulldate);
					}
            ...
            let payotal = form.find('[name="pay_total_amount"]').val();
            if (elementId == 'pay_amount') {
                if (payotal != '' && payotal != undefined && payotal != null) {
                    let am = parseFloat(payotal) / years;
                    html.find('#' + expectedElementId).val(am.toFixed(2));
                }
            }
        }
        $('.lineitemTableContainerAno .appendRows').append(html);
        $('#pay_sch_sl_no0').attr('name', 'pay_sch_sl_no' + j);
        $('#payment_date0').attr('name', 'schedule_date' + j);
        $('#pay_amount0').attr('name', 'pay_amount' + j);
        $('input[name^="schedule_date"]').datepicker({
        });
    }
    $(".lineitemTableContainerAno .appendRows tr:first-child").remove();
},
	...
	AddContractLine: function () {
		...
		// Event listener for changes in payment_frequency and pm_month
		// form.find('select[name="payment_frequency"], select[name="pm_month"]').on('change', function () {
		// 	let pmMonth = parseInt(form.find('[name="pm_month"]').val());
		// 	let paymentFrequency = form.find('select[name="payment_frequency"]').val();

		// 	// Calculate installments based on month and frequency
		// 	let installments = 0;
		// 	if (pmMonth === 6) {
		...
		// 	// Update the no_of_installments field
		// 	form.find('[name="no_of_installments"]').val(installments);

		// 	// Trigger the handlePaySchedule function to update the payment schedule
		// 	self.handlePaySchedule();
		// });

		jQuery('#PeriodicalMaintainence_editView_fieldName_no_of_installments').on('input', function () {
			self.handlePaySchedule();
		});
		jQuery('#PeriodicalMaintainence_editView_fieldName_payment_start_date').on('change', function () {
			self.handlePaySchedule();
		});
		$('select[name="payment_frequency"').on('change', function () {
			self.handlePaySchedule();
		});
		jQuery('#PeriodicalMaintainence_editView_fieldName_pay_total_amount').on('input', function () {
			self.handlePaySchedule();
		});
	},
    ...
    registerReferenceSelectionEvent1: function (container) {
        var self = this;
        jQuery('input[name="serialnumber"]', container).on(Vtiger_Edit_Js.referenceSelectionEvent, function (e, data) {
            self.autofillAssetName();
        });
    },

    //Jagriti Singh
    
    clickdateIcon: function () {
        $(document).on("click", ".input-group-addon", function () {
            $(this).closest(".input-group").find("input.dateField").focus();
        });
    },

    registerEventForAssetAutofill: function () {
      
        let self = this;
        // alert('start');
        var form = this.getForm();
        form.find('input[name="serial_no"]').on(Vtiger_Edit_Js.referenceSelectionEvent, function(e, data) {
    ...
    autofillAllDeatails: function () {
        var form = this.getForm();
        var assetId = form.find('[name="serial_no"]').val();
        ...
        if (assetId) {
            ...
            data = response.data
                if (!error && response) {
                    // form.find('[name="serial_no"]').val(data.serial_no);
                    form.find('[name="serial_no_display"]').val(data.serialnumber);
                    form.find('[name="assetname"]').val(data.assetname);
                    // form.find('[name="product"]').val(data.product);
                    // form.find('[name="purchase_date"]').val(data.purchase_date);
                    // form.find('[name="warranty_expiry"]').val(data.warranty_expiry);
    ...
    registerEventForPmEndDate: function () {
        var self = this;
        var form = this.getForm();

        form.find('[name="pm_month"]').on('change', function () {
            self.autofillPmEndDate();
        });

        form.find('[name="payment_start_date"]').on('change', function () {
            self.autofillPmEndDate();
        });
    },

    /**
     * Function to calculate and autofill pm_enddate based on pm_month and payment_start_date
     */
    autofillPmEndDate: function () {
        var form = this.getForm();
        var pmMonth = parseInt(form.find('[name="pm_month"]').val());
        var paymentStartDate = form.find('[name="payment_start_date"]').val();
    
        if (!paymentStartDate || isNaN(pmMonth)) {
            return;
        }   
    
        var dateParts = paymentStartDate.split("-");
        var startDate = new Date(dateParts[2], dateParts[1] - 1, dateParts[0]);
    
        // Add months to startDate
        startDate.setMonth(startDate.getMonth() + pmMonth);
        ...
        form.find('[name="pm_enddate"]').val(endDate);
    },
	...
	lineItemsHolder3: false,
	numOfLineItems: false,
	numOfLineItems1: false,
	numOfLineItems3: false,
	dummyLineItemRow3: false,
	...
	initializeVariables: function () {
		...
		this.lineItemsHolder3 = jQuery('#lineItemTab3');
		this.dummyLineItemRow3 = jQuery('#row3r0');
		...
		this.numOfLineItems3 = this.lineItemsHolder3.find('.' + this.lineItemDetectingClass).length;
	...
	getLineItemNextRowNumber3: function () {
		console.log("karma");
		console.log(++this.numOfLineItems3);
		return ++this.numOfLineItems3;
	},
	...
	checkLineItemRow: function () {
		...
		anotherNumRows = this.lineItemsHolder3.find('.' + this.lineItemDetectingClass).length;
		if (anotherNumRows > 0) {
			this.showLineItemsDeleteIcon3();
		} else {
			this.hideLineItemsDeleteIcon3();
		}
	},

	showLineItemsDeleteIcon3: function () {
		this.lineItemsHolder1.find('.deleteRow').show();
	},

	hideLineItemsDeleteIcon3: function () {
	...
	saveProductCount3 : function () {
		jQuery('#totalProductCount3').val(this.lineItemsHolder3.find('tr.' + this.lineItemDetectingClass).length);
	},
	...
	updateRowNumberForRow: function (lineItemRow, expectedSequenceNumber, currentSequenceNumber) {
		...
		var idFields = new Array('engineer_id_line','asset_id_line','productName', 'subproduct_ids', 'hdnProductId', 'purchaseCost', 'margin', 'productName_other', 'qty_other',
			'comment', 'qty','pm_zone','pm_area','listPrice', 'discount_div', 'discount_type', 'hdnProductId_other',
		...
		if (fildNamesOfCustFields && fildNamesOfCustFields.length > 0) {
		...
		lineItemRow.find('[data-fieldname="' + 'engineer_id_line0_display' + '"]').attr('name', 'engineer_id_line' + expectedSequenceNumber + '_display');
		lineItemRow.find('[data-fieldname="' + 'asset_id_line0_display' + '"]').attr('name', 'asset_id_line' + expectedSequenceNumber + '_display');
	...
	initializeLineItemRowCustomFields: function (lineItemRow, rowNum) {
		...
		console.log(fildNamesOfCustFields);
		if (fildNamesOfCustFields == null || fildNamesOfCustFields == undefined) {
			fildNamesOfCustFields = '[]';
		}
		fildNamesOfCustFields = JSON.parse(fildNamesOfCustFields);
		let pickLength = 0;//fildNamesOfCustFields.length;
	...
	getNewLineItem3: function (params) {
		var currentTarget = params.currentTarget;
		var itemType = currentTarget.data('moduleName');
		var newRow = this.dummyLineItemRow3.clone(true).removeClass('hide').addClass(this.lineItemDetectingClass).removeClass('lineItemCloneCopy');
		var individualTax = this.isIndividualTaxMode();
		if (individualTax) {
			newRow.find('.individualTaxContainer').removeClass('hide');
		}
		newRow.find('.lineItemPopup').filter(':not([data-module-name="' + itemType + '"])').remove();
		newRow.find('.lineItemType').val(itemType);
		var newRowNum = this.getLineItemNextRowNumber3();
		this.updateRowNumberForRow(newRow, newRowNum);
		this.initializeLineItemRowCustomFieldsOther(newRow, newRowNum);
		return newRow
	},
	...
	mapResultsToFields3: function (parentRow, responseData) {
		var lineItemNameElment = jQuery('input.productName', parentRow);
		var referenceModule = this.getLineItemSetype(parentRow);
		var lineItemRowNumber = parentRow.data('rowNum');
		for (var id in responseData) {
	...
	registerAddProductService: function () {
		...
		var addLineItemEventHandler3 = function (e, data) {
			var currentTarget = jQuery(e.currentTarget);
			var params = { 'currentTarget': currentTarget }
			var newLineItem = self.getNewLineItem3(params);
			newLineItem = newLineItem.appendTo(self.lineItemsHolder3);
			newLineItem.find('input.productName').addClass('autoComplete1');
			newLineItem.find('.ignore-ui-registration').removeClass('ignore-ui-registration');
			vtUtils.applyFieldElementsView(newLineItem);
			app.event.trigger('post.lineItem.New', newLineItem);
			...
			if (typeof data != "undefined") {
				console.log(data);
				self.mapResultsToFields3(newLineItem, data);
			}
		}

		jQuery('#addProduct3').on('click', addLineItemEventHandler3);
		...
		input.addEventListener('blur', function () {
			...
			if (input.value.trim()) {
				if (intl.isValidNumber()) {
					...
					$('#PeriodicalMaintainence_editView_fieldName_phone').val(num);
