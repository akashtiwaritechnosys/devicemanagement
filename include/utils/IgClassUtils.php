...
class IgClassUtils {
    ...
    static function saveLineDetailsEquipment($total_year_cont, $recordId, $module) {
        global $log, $adb;
        $log->debug("***** Entering Into Save LIne Item ***********");
        $table = 'vtiger_sales_order_pay_schedule';
    ...
    static function saveLineDetailsEquipment1($total_year_cont, $recordId, $module) {
        global $log, $adb;
        $log->debug("***** Entering Into Save LIne Item ***********");
        $table = 'vtiger_pm_schedule';
        $adb->pquery("delete from $table where id=?", array($recordId));
        $prod_seq = 1;
        global $current_user;
        $formate = $current_user->date_format;
    ...
    $recordInfo, $ticketCreatedDateTime, $notiType, $id, $exterAppNum, $igModule
    ) {
        global $adb;
        global $log;
        ...
        if ($recordInfo['sr_ticket_type'] == 'BREAKDOWN' || $recordInfo['sr_ticket_type'] == 'ERECTION AND COMMISSIONING' || $recordInfo['sr_ticket_type'] == 'PRE-DELIVERY') {
            ...
            if (
                    $recordInfo['sr_ticket_type'] == 'ERECTION AND COMMISSIONING' || $recordInfo['sr_ticket_type'] == 'PRE-DELIVERY'
            ) {
