...
function getOnlyLinkedEquimentsSEHelpDesk($recordId) {
	$belowRoleContactIds = [];
	array_push($belowRoleContactIds, $recordId);
	global $adb;
	$sql = "SELECT ticketid FROM `vtiger_troubletickets` " .
		" inner join vtiger_crmentity on vtiger_crmentity.crmid=vtiger_troubletickets.ticketid " .
		" inner join vtiger_userrel_eng on vtiger_userrel_eng.id=vtiger_troubletickets.ticketid " .
		" where `vtiger_userrel_eng`.engineer_id_line  in (" . generateQuestionMarks($belowRoleContactIds) . ") and vtiger_crmentity.deleted = 0";
	$result = $adb->pquery($sql, $belowRoleContactIds);
	$recordIds = [];
	while ($row = $adb->fetch_array($result)) {
		array_push($recordIds, $row['ticketid']);
	}
	return $recordIds;
}
