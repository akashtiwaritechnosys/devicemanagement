...
function IgCalculateWarranty($id, $text, $currhmr, $warrentyStartDate, $currStatus) {
	...
	if (!empty($dataValues["month"]) && !empty($warrentyStartDate)) {
		...
		if ($warrentyEndDate > date("Y-m-d")) {
			...
			ValueUpdateInDB($currStatus, $id, $warrentyEndDate);
		} else {
...
function ValueUpdateInDB($currStatus, $id, $warrantyEndDate, $daysLeftInWarranty, $activeStatus) {
	$db = PearDatabase::getInstance();
	$db->pquery("UPDATE vtiger_equipment set eq_run_war_st=? , war_end_date = ?,
	days_left_in_war=?, warranty_status = ?
	where equipmentid=?", array(
		$currStatus,
		$warrantyEndDate,
		$daysLeftInWarranty,
		$activeStatus,
...
function InsertDatabase($status, $amcStatus, $id) {
	$db = PearDatabase::getInstance();
	$db->pquery(
		"UPDATE vtiger_equipment set eq_run_war_st = ?,amc_status = ? where equipmentid=?",
		array($status, $amcStatus, $id)
	);
}
